...
export function BrothersPage() {
  ...
  return (
    <section className="directory-page">
      ...
      {favorites.length === 0 ? <div className="panel empty-state"><h2>Пока никого не сохранил</h2><p>Нажимай на звезду в карточках участников — они появятся здесь.</p></div> : <div className="directory-grid">{favorites.map((profile) => <ProfileCard key={profile.id} profile={profile} favorite onOpen={() => setOpened(profile)} onToggleFavorite={() => toggleFavorite(profile.id)} />)}</div>}
